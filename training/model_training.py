...
def train(epoch, model, loss_function, optimizer, features_train, y_train, hparams):
    ...
    with tqdm(range(0, N_train, batch_size), unit='batch') as tepoch:
...
def validate(epoch, model, loss_function, features_val, y_val, hparams):
    ...
    model.eval()
    with torch.no_grad():
        for i_batch in range(0, N_val, batch_size):
            adj_batch = list()
            features_batch = list()
            idx_batch = list()
            y_batch = list()
            ...
            # Create tensors
            for j in range(i_batch, min(N_val, i_batch + batch_size)):
                n = adj_val[j].shape[0]
                adj_batch.append(adj_val[j] + sp.identity(n))
                features_batch.append(node_features_val[j])
                idx_batch.extend([j - i_batch] * n)
...
def run(hparams, experiment_name=None, seed=42):
    ...
    for epoch in range(hparams['epochs']):
        train_loss = train(epoch, model, loss_function, optimizer, features_train, y_train, hparams)
        val_loss = validate(epoch, model, loss_function, features_val, y_val, hparams)
        session.report({
